...
```{r}
left_join(PoliceDistrictBoundaries_Current, District_Summaries)
```
...
```{r}
centroids <- st_centroid(PoliceDistrictBoundaries_Current

ggplot(left_join(PoliceDistrictBoundaries_Current, District_Summaries)) +
  geom_sf(aes(fill = Crime_per_1000, alpha = 0.6)) +
  geom_text(data = centroids, aes(label = District, geometry = geometry), stat = "sf_coordinates", size = 2.5, color = "black") +
  scale_fill_viridis_c(
  option = "plasma",
  name = "Crimes per 1,000",
  breaks = breaks,
